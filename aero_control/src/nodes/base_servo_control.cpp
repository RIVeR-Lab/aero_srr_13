...
void BaseServoController::UpdateError(void) {
	...
	tf_listener.waitForTransform("/workspace", this->desired_pose.header.frame_id, this->desired_pose.header.stamp,
			ros::Duration(0.1));
	tf_listener.transformPose("/workspace", this->desired_pose, error_pose);
...
int main(int argc, char **argv) {
	...
	if (argc < 1) {
		ROS_INFO( "Usage: base_servo_control desired_position_topic base_velocity_topic");
