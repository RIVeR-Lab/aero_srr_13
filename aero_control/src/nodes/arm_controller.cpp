...
ArmController::ArmController(ros::NodeHandle nh, ros::NodeHandle param_nh) {
	...
	/* Messages */
	this->sub_object_position = nh.subscribe(ObjectPose, 1, &ArmController::ObjectPosition, this);
...
void ArmController::ObjectPosition(const aero_srr_msgs::ObjectLocationMsgConstPtr& object) {
	...
	listener.waitForTransform("arm_base", object->pose.header.frame_id, object->pose.header.stamp,
			ros::Duration(1.0));
