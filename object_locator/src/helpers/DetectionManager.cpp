...
void DetectionManager::shrink() {
	DetectionArray_t::iterator itr = this->detections_.begin();
	while (itr != this->detections_.end()) {
...
bool DetectionManager::getDetection(tf::Point& detection, object_type &type,
...
bool DetectionManager::getAllAboveConf(
		std::vector<tf::Point>& detections) const {
	if (this->size() > 0) {
		BOOST_FOREACH(DetectionArray_t::value_type item, this->detections_) {
			if (item->second > this->threshold_det_)
